------------------------------
## 📡 Лечим Wi-Fi на RTL8723BE (Linux) [1, 2] 
Симптомы: пропадает сеть, ошибка Init MAC failed или No such device (-19).
------------------------------
## 🛠 1. Настройка ядра (ASPM) — Критично
Отключаем управление питанием шины PCIe, которое часто «вешает» этот чип.

   1. Откройте конфиг: sudo nano /etc/default/grub
   2. В строку GRUB_CMDLINE_LINUX_DEFAULT добавьте pcie_aspm=off.
   Должно быть: "quiet splash pcie_aspm=off"
   3. Обновите загрузчик: sudo update-grub и перезагрузитесь. [3] 

------------------------------
## ⚙️ 2. Конфигурация драйвера
Настраиваем выбор антенны и отключаем программный сон.
Создайте файл:

sudo nano /etc/modprobe.d/rtl8723be.conf

Добавьте строку:

options rtl8723be ant_sel=2 ips=0 fwlps=0 swenc=1 msi=0

Примечание: если сигнал слабый, попробуйте изменить ant_sel=2 на ant_sel=1. [4] 
------------------------------
## ⚡ 3. Отключение энергосбережения
Чтобы система не «усыпляла» адаптер во время работы.
В NetworkManager:

sudo nano /etc/NetworkManager/conf.d/wifi-powersave.conf

Добавьте:

[connection]
wifi.powersave = 2

В реальном времени (для интерфейса):

sudo iw dev wlp3s0 set power_save off

(Замените wlp3s0 на имя вашего интерфейса из iw dev).
------------------------------
## 📻 4. Дополнительные меры

* Bluetooth: Если Wi-Fi всё равно падает, отключите Bluetooth (они на одном чипе): sudo systemctl disable --now bluetooth.
* Перезапуск модуля: Чтобы применить настройки без ребута:

sudo modprobe -r rtl8723be && sudo modprobe rtl8723be


------------------------------
## ✅ Как проверить, что помогло?

* lsmod | grep rtl8723be — драйвер в системе.
* dmesg | grep -i rtl — лог ошибок. Если видите boot from eeprom — чип инициализировался успешно. [5] 

Итог: Если ошибки Init MAC failed сохраняются после всех шагов — это признак физической деградации чипа. В таком случае поможет только замена на Intel или USB-«свисток».
------------------------------
Нужно ли уточнить, как определить номер антенны (1 или 2), если сигнал останется очень слабым?


# 📡 Исправление разрывов Wi-Fi на Linux (RTL8723BE)
Решение проблем с нестабильным соединением для адаптеров **Realtek RTL8723BE**.
---## 🧩 Проблема
На ноутбуках с чипом **RTL8723BE** Wi-Fi может:
- Случайно отключаться.
- Показывать слабый или нестабильный сигнал.
- Исчезать из системы с ошибкой `No such device (-19)`.
- Постоянно переподключаться.

**Основные причины:
**1. Неправильный выбор антенны драйвером (их две, но задействована может быть одна).
2. Агрессивное энергосбережение Linux.
3. Нестабильная работа стандартного драйвера Realtek.
---## 💻 Совместимое оборудование
Инструкция применима к любым ноутбукам с модулем **RTL8723BE**, например:
- **ASUS VivoBook Max**
- HP Pavilion / Compaq
- Lenovo Ideapad
---## ✅ Решение (Стабильная конфигурация)### 1. Настройка драйвера (Исправление антенны)
Создайте файл конфигурации:
```bash
sudo nano /etc/modprobe.d/rtl8723be.conf
```
Добавьте в него следующую строку:
```bash
options rtl8723be ant_sel=2 ips=0 fwlps=0
```
🔧 Разбор параметров:

* `ant_sel=2` — принудительно выбирает вторую антенну (критически важное исправление).
* `ips=0` — отключает программное энергосбережение (Idle Power Save).
* `fwlps=0` — отключает энергосбережение на уровне прошивки (Firmware Low Power State).

------------------------------
## 2. Применение изменений
Перезагрузите модуль драйвера:
```bash
sudo modprobe -r rtl8723be
sudo modprobe rtl8723be
```
Или просто выполните `reboot`.
------------------------------
## 3. Отключение энергосбережения в NetworkManager
Создайте конфиг:
```bash
sudo nano /etc/NetworkManager/conf.d/wifi-powersave.conf
```
Добавьте:
```bash
[connection]
wifi.powersave = 2
```
Примените настройки:
```bash
sudo systemctl restart NetworkManager
```
------------------------------
## 4. Отключение Power Save в реальном времени (Runtime)
Установите инструмент iw:
```bash
sudo apt install iw
```
Узнайте имя вашего интерфейса:
```bash
iw dev
```
Отключите спящий режим (замените wlp3s0 на имя вашего интерфейса):
```bash
sudo iw dev wlp3s0 set power_save off
```
------------------------------
## 🔍 Проверка параметров
Убедитесь, что настройки применились:
```bash
cat /sys/module/rtl8723be/parameters/ant_sel
cat /sys/module/rtl8723be/parameters/ips
cat /sys/module/rtl8723be/parameters/fwlps
```
Ожидаемый результат: `2, N, N`.
------------------------------
## 🧪 Диагностика (Troubleshooting)
Если Wi-Fi всё еще не работает, проверьте следующее:

* Наличие интерфейса: `ip link` или `iw dev`
* Загружен ли драйвер: 
```bash
lsmod | grep rtl8723be
```
* Системные логи: 
```bash
dmesg | grep -i rtl
```
------------------------------
## ⚠️ Важные примечания

* Параметр `ant_sel` зависит от конкретной модели ноутбука. Если сигнал всё еще слабый, попробуйте изменить значение на `ant_sel=1`.
* Драйверы Realtek для старых чипов часто работают нестабильно на новых ядрах Linux.

------------------------------
## 🚀 Рекомендуемая альтернатива
Если программные методы не помогают, лучшим решением будет:

* Замена модуля на Intel или Atheros (если позволяет BIOS ноутбука).
* Использование внешнего USB Wi-Fi адаптера с хорошей поддержкой Linux.

------------------------------
## 📌 Резюме

| Исправление | Статус |
|---|---|
| Тюнинг ant_sel | ✅ Критично |
| Отключение энергосбережения | ✅ Важно |
| Перезагрузка драйвера | ✅ Необходимо |

------------------------------
## 📜 Лицензия
MIT / Свободно для использования.




---

# Настройка wi‑fi на Asus VivoBook Max

Драйвер: для драйвера rtl8723be
*если другой - заменить имя драйвера

Проверить модель адаптера Wi‑Fi:
```
lspci -nnk | grep -iA3 network
```

Создать или открыть файл конфигурации модуля:
```
sudo nano /etc/modprobe.d/rtl8723be.conf
```

Вставить одну строку:
```
options rtl8723be ant_sel=2 fwlps=0 ips=0
```

Если после этого на некоторых моделях Asus сигнал стал хуже, попробуйте другую антенну, поменяв параметр на:
```
options rtl8723be ant_sel=1 fwlps=0 ips=0
```

Применить изменения (перезагрузка):
```
sudo reboot
```

Или без перезагрузки — перезагрузить модуль:
```
sudo modprobe -r rtl8723be
sudo modprobe rtl8723be
```

Коротко о параметрах:
- ant_sel — выбор антенны (1 или 2);
- fwlps, ips — отключение режимов энергосбережения (0 — выключено).

Используйте эти команды с правами администратора и сохраняйте резервные копии конфигураций перед изменениями.
